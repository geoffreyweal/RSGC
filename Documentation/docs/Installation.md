...
In this article, we will look at how to install the RSGC program and all requisites required for this program.
...
!!! note
	...
	In any case, RSGC does not use the CSD Python API at all. Only [ACSD](https://geoffreyweal.github.io/ACSD) reads directly from the CSD. 
...
The RSGC program uses the Atomic Simulation Environment (ASE) to read in the crystal data from various file format, to process the crystals, and to save the the crystals to disk. Read more about [ASE here](https://wiki.fysik.dtu.dk/ase). 
...
``Networkx`` is a python program that is used in the RSGC program to describe the bonding structure between atoms in the crystal structure. The easiest way to install ``Networkx`` is though ``pip``. Type the following into the terminal:
...
## Setting up the RSGC Program

There are three ways to install RSGC on your system. These ways are described below:


### Install RSGC through ``pip3``

To install the RSGC program using ``pip3``, perform the following in your terminal:

```bash
pip3 install --upgrade --user git+https://github.com/geoffreyweal/RSGC.git
```
...
### Install RSGC through ``conda``

You can install the RSGC program on ``conda`` through ``pip``. [Click here](https://conda.io/projects/conda/en/latest/user-guide/tasks/manage-pkgs.html#installing-non-conda-packages) to see more information about installing RSGC through ``conda``.


### Manual installation

First, download the RSGC program to your computer. You can do this by cloning a version of this from Github, or obtaining a version of the program from the authors. If you are obtaining this program via Github, you want to ``cd`` to the directory that you want to place this program in on the terminal, and then clone the program from Github through the terminal as well: 

```bash
cd PATH/TO/WHERE_YOU_WANT_TO_PLACE_RSGC_ON_YOUR_COMPUTER
...
!!! tip
	...
	Make sure that the path given to ``PATH_TO_RSGC`` is the correct path to the RSGC folder. 
...
Once you have run ``source ~/.bashrc``, the RSGC program should be all ready to go!

RSGC is a python library rather than a terminal command, so there is no ``RSGC`` executable to look for. Check it by importing it instead:

```bash
python3 -c "import RSGC; print(RSGC.__file__)"
```

This should give you the path to the RSGC package. If python reports ``ModuleNotFoundError``, check that the path you gave for ``PATH_TO_RSGC`` is the correct path to the RSGC folder. 
...
where ``"<Path_to_RSGC>"`` is the directory path that you place the RSGC program. You can find this by changing directory (``cd``) into the RSGC folder and typing ``pwd`` into the terminal. This will give you the full path to the RSGC program. 
